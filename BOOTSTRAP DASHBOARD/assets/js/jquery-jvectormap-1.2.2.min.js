...
var jvm = {
	...
	min: function (e) {
		var t = Number.MAX_VALUE,
			n;
		if (e instanceof Array) for (n = 0; n < e.length; n++) e[n] < t && (t = e[n]);
		else for (n in e) e[n] < t && (t = e[n]);
		return t;
	},
	max: function (e) {
...
(jvm.$ = jQuery),
	...
	(jvm.AbstractElement.prototype.set = function (e, t) {
		var n;
		if (typeof e == 'object') for (n in e) (this.properties[n] = e[n]), this.applyAttr(n, e[n]);
	...
	(jvm.VMLCanvasElement.prototype.applyTransformParams = function (e, t, n) {
		(this.scale = e),
			...
			(this.rootElement.node.coordorigin =
				this.width - t - this.width / 100 + ',' + (this.height - n - this.height / 100)),
	...
	(jvm.VMLShapeElement.prototype.applyAttr = function (e, t) {
		switch (e) {
			...
			case 'stroke-opacity':
				...
				break;
			case 'stroke-width':
				parseInt(t, 10) === 0 ? (this.node.stroked = !1) : (this.node.stroked = !0),
	...
	(jvm.VMLPathElement.pathSvgToVml = function (e) {
		...
		return (
			...
			e
				.replace(/([MmLlHhVvCcSs])\s*((?:-?\d*(?:\.\d+)?\s*,?\s*)+)/g, function (e, t, o, u) {
					(o = o
						.replace(/(\d)-/g, '$1,-')
						.replace(/^\s+/g, '')
						.replace(/\s+$/g, '')
	...
	(jvm.NumericScale.prototype = {
		setMin: function (e) {
			(this.clearMinValue = e),
				typeof this.normalize == 'function' ? (this.minValue = this.normalize(e)) : (this.minValue = e);
		},
		setMax: function (e) {
			(this.clearMaxValue = e),
				typeof this.normalize == 'function' ? (this.maxValue = this.normalize(e)) : (this.maxValue = e);
		},
		setScale: function (e) {
			var t;
			...
			return (
				i == this.scale.length - 1
					...
					: (e = this.vectorToNum(
							this.vectorAdd(
								this.scale[i],
								this.vectorMult(this.vectorSubtract(this.scale[i + 1], this.scale[i]), e / t[i])
							)
					  )),
				e
			);
	...
	(jvm.DataSeries = function (e, t) {
		...
		(e = e || {}),
			...
			jvm.$.isArray(e.scale)
				? ((n = e.attribute === 'fill' || e.attribute === 'stroke' ? jvm.ColorScale : jvm.NumericScale),
				  (this.scale = new n(e.scale, e.normalizeFunction, e.min, e.max)))
				: e.scale
	...
	(jvm.DataSeries.prototype = {
		setAttributes: function (e, t) {
			...
			else for (r in n) this.elements[r] && this.elements[r].element.setStyle(this.params.attribute, n[r]);
		},
		setValues: function (e) {
			...
			if (this.scale instanceof jvm.OrdinalScale || this.scale instanceof jvm.SimpleScale)
				...
				if (!this.params.min || !this.params.max) {
					...
					this.params.min || this.scale.setMin(n),
						this.params.max || this.scale.setMax(t),
						(this.params.min = n),
						(this.params.max = t);
				}
				for (i in e)
	...
	(jvm.Proj = {
		...
		mill: function (e, t, n) {
			return {
				...
				y: (-this.radius * Math.log(Math.tan((45 + 0.4 * e) * this.radDeg))) / 0.8,
			};
		},
		mill_inv: function (e, t, n) {
			return {
				lat: (2.5 * Math.atan(Math.exp((0.8 * t) / this.radius)) - (5 * Math.PI) / 8) * this.degRad,
		...
		aea: function (e, t, n) {
			...
			return { x: h * Math.sin(c) * this.radius, y: -(p - h * Math.cos(c)) * this.radius };
		},
		aea_inv: function (e, t, n) {
			var r = e / this.radius,
				...
				p = Math.atan(r / (c - i));
			return { lat: Math.asin((l - h * h * f * f) / (2 * f)) * this.degRad, lng: (o + p / f) * this.degRad };
		},
		lcc: function (e, t, n) {
			var r = 0,
				i = n * this.radDeg,
				...
				a = e * this.radDeg,
				f =
					Math.log(Math.cos(o) * (1 / Math.cos(u))) /
					Math.log(Math.tan(Math.PI / 4 + u / 2) * (1 / Math.tan(Math.PI / 4 + o / 2))),
			...
			return { x: c * Math.sin(f * (s - i)) * this.radius, y: -(h - c * Math.cos(f * (s - i))) * this.radius };
		},
		lcc_inv: function (e, t, n) {
			var r = e / this.radius,
				...
				a = 45 * this.radDeg,
				f =
					Math.log(Math.cos(u) * (1 / Math.cos(a))) /
					Math.log(Math.tan(Math.PI / 4 + a / 2) * (1 / Math.tan(Math.PI / 4 + u / 2))),
	...
	(jvm.WorldMap = function (e) {
		...
		(this.canvas = new jvm.VectorCanvas(this.container[0], this.width, this.height)),
			'ontouchstart' in window || (window.DocumentTouch && document instanceof DocumentTouch)
				? this.params.bindTouchEvents && this.bindContainerTouchEvents()
			...
			this.setSize(),
			this.params.focusOn &&
				(typeof this.params.focusOn == 'object'
	...
	(jvm.WorldMap.prototype = {
		...
		applyTransform: function () {
			...
			this.defaultWidth * this.scale <= this.width
				...
				this.defaultHeight * this.scale <= this.height
					...
					: ((t = 0), (r = (this.height - this.defaultHeight * this.scale) / this.scale)),
				this.transY > t ? (this.transY = t) : this.transY < r && (this.transY = r),
					...
					a.type == 'touchstart' && (u = 0),
						f.length == 1
							? (u == 1 &&
									...
									n.applyTransform(),
									n.label.hide(),
									(h != n.transX || p != n.transY) && a.preventDefault()),
							  (r = f[0].pageX),
							  (i = f[0].pageY))
							: f.length == 2 &&
							  (u == 2
									? ((c =
											Math.sqrt(
												Math.pow(f[0].pageX - f[1].pageX, 2) +
													Math.pow(f[0].pageY - f[1].pageY, 2)
											) / t),
									  n.setScale(e * c, s, o),
									  n.label.hide(),
									  a.preventDefault())
									: ((l = jvm.$(n.container).offset()),
									  ...
									  (e = n.scale),
									  (t = Math.sqrt(
											Math.pow(f[0].pageX - f[1].pageX, 2) + Math.pow(f[0].pageY - f[1].pageY, 2)
									  )))),
						(u = f.length);
				};
		...
		bindElementEvents: function () {
			...
			this.container.mousemove(function () {
				...
				this.container.delegate("[class~='jvectormap-element']", 'mouseover mouseout', function (t) {
					...
					t.type == 'mouseover'
						? (e.container.trigger(f, [s]),
						  f.isDefaultPrevented() || o.setHovered(!0),
						  e.label.text(u),
						  e.container.trigger(a, [e.label, s]),
						  a.isDefaultPrevented() ||
								(e.label.show(), (e.labelWidth = e.label.width()), (e.labelHeight = e.label.height())))
						: (o.setHovered(!1), e.label.hide(), e.container.trigger(i + 'Out.jvectormap', [s]));
				}),
				...
				this.container.delegate("[class~='jvectormap-element']", 'mouseup', function (n) {
					...
					if (!t) {
						...
						if (
							(s === 'region' && e.params.regionsSelectable) ||
							(s === 'marker' && e.params.markersSelectable)
						)
							u.isDefaultPrevented() ||
								(e.params[s + 'sSelectableOne'] && e.clearSelected(s + 's'),
		...
		createLabel: function () {
			...
			(this.label = jvm.$('<div/>').addClass('jvectormap-label').appendTo(jvm.$('body'))),
				this.container.mousemove(function (t) {
					...
					n < 5 && (n = t.pageX + 15),
						r < 5 && (r = t.pageY + 15),
						e.label.is(':visible') && e.label.css({ left: n, top: r });
				});
		},
		setScale: function (e, t, n, r) {
			...
			e > this.params.zoomMax * this.baseScale
				? (e = this.params.zoomMax * this.baseScale)
				: e < this.params.zoomMin * this.baseScale && (e = this.params.zoomMin * this.baseScale),
				typeof t != 'undefined' &&
					typeof n != 'undefined' &&
					((i = e / this.scale),
					r
						? ((this.transX = t + (this.defaultWidth * (this.width / (this.defaultWidth * e))) / 2),
						  (this.transY = n + (this.defaultHeight * (this.height / (this.defaultHeight * e))) / 2))
						: ((this.transX -= ((i - 1) / e) * t), (this.transY -= ((i - 1) / e) * n))),
				(this.scale = e),
				this.applyTransform(),
		...
		setFocus: function (e, t, n) {
			...
			if (jvm.$.isArray(e) || this.regions[e]) {
				...
				for (u = 0; u < o.length; u++)
					this.regions[o[u]] &&
						((i = this.regions[o[u]].element.getBBox()),
						i &&
							(typeof r == 'undefined'
								? (r = i)
								: ((s = {
								  ...
								  (r = s))));
				this.setScale(
					Math.min(this.width / r.width, this.height / r.height),
					-(r.x + r.width / 2),
					-(r.y + r.height / 2),
					!0
				);
		...
		setSelected: function (e, t) {
			var n;
			typeof t != 'object' && (t = [t]);
			if (jvm.$.isArray(t)) for (n = 0; n < t.length; n++) this[e][t[n]].element.setSelected(!0);
			else for (n in t) this[e][n].element.setSelected(!!t[n]);
		},
		...
		createRegions: function () {
			...
			for (e in this.mapData.paths)
				(t = this.canvas.addPath(
					...
					jvm.$.extend(!0, {}, this.params.regionStyle)
				)),
					jvm.$(t.node).bind('selected', function (e, t) {
						n.container.trigger('regionSelected.jvectormap', [
							jvm.$(this).attr('data-code'),
							t,
							n.getSelectedRegions(),
						]);
					}),
					t.addClass('jvectormap-region jvectormap-element'),
					(this.regions[e] = { element: t, config: this.mapData.paths[e] });
		},
		createMarkers: function (e) {
			...
			for (t in e)
				(i = e[t] instanceof Array ? { latLng: e[t] } : e[t]),
					(r = this.getMarkerPosition(i)),
					r !== !1 &&
						((n = this.canvas.addCircle(
							{ 'data-index': t, cx: r.x, cy: r.y },
							jvm.$.extend(!0, {}, this.params.markerStyle, { initial: i.style || {} }),
							this.markersGroup
						)),
						n.addClass('jvectormap-marker jvectormap-element'),
						jvm.$(n.node).bind('selected', function (e, t) {
							o.container.trigger('markerSelected.jvectormap', [
						...
						(this.markers[t] = { element: n, config: i }));
		},
		repositionMarkers: function () {
			var e, t;
			for (e in this.markers)
				(t = this.getMarkerPosition(this.markers[e].config)),
					t !== !1 && this.markers[e].element.setStyle({ cx: t.x, cy: t.y });
		},
		getMarkerPosition: function (e) {
			return jvm.WorldMap.maps[this.params.map].projection
		...
		latLngToPoint: function (e, t) {
			...
			return (
				...
				(u = this.getInsetForPoint(n.x, n.y)),
				u
					? ((a = u.bbox),
					  (n.x = ((n.x - a[0].x) / (a[1].x - a[0].x)) * u.width * this.scale),
